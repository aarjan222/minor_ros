...
namespace ackermann_drive_controller
{
      ...
      const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    ...
    {
      RCLCPP_WARN_ONCE(logger, "Maximum turning rate limit exceeded: %f %f %f %f", angular_command, angular_command_limit, fabs(linear_command), steer.min_turning_radius);
      angular_command = angular_command_limit;
    }
    else if (angular_command < -angular_command_limit)
    {
      RCLCPP_WARN_ONCE(logger, "Minimum turning rate limit exceeded: %f %f %f %f", angular_command, -angular_command_limit, fabs(linear_command), steer.min_turning_radius);
      angular_command = -angular_command_limit;
    }
    ...
    // RCLCPP_INFO(logger, "Velocity l and r: %f\t%f", velocity_left, velocity_right);
    // RCLCPP_INFO(logger, "Steering angle l and r : %f\t%f", alpha1, alpha2);
